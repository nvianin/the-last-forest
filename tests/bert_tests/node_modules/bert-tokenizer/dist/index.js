...
function runStripAccents(text) {
    ...
    nsmarks.forEach(function (cp) { text = text.replace(String.fromCodePoint(cp), ""); });
    return text;
}
...
function runSplitOnPunctuation(text) {
    var output = [];
    var preCodePoint = -1;
    var preCodePointIsPunc = false;
    for (var i = 0; i < text.length; i++) {
        var codePoint = text.charCodeAt(i);
        var isPuncCurrent = isPunctuation(codePoint);
...
var BertTokenizer = (function () {
    function BertTokenizer(pathToVocabulary, doLowerCase, maxSeqLength) {
        if (pathToVocabulary === void 0) { pathToVocabulary = DEFAUT_VOCAB_PATH; }
        if (doLowerCase === void 0) { doLowerCase = true; }
        if (maxSeqLength === void 0) { maxSeqLength = 128; }
        this.tokenizer = new wordpiecetokenizer_1.default();
    ...
    BertTokenizer.prototype.convertSingleExample = function (text) {
        var inputIds = [];
        var inputMask = [];
        var segmentIds = [];
        var tokenIds = this.tokenize(text);
        inputIds.push(this.clsId);
        inputMask.push(1);
        segmentIds.push(0);
        inputIds.push.apply(inputIds, tokenIds);
        ...
        inputMask.push(1);
        segmentIds.push(0);
        var numTokens = inputIds.length;
        for (var i = 0; i < this.maxSeqLength - numTokens; i++) {
            inputIds.push(0);
            inputMask.push(0);
            segmentIds.push(0);
        }
        return { inputIds: inputIds, segmentIds: segmentIds, inputMask: inputMask };
    };
